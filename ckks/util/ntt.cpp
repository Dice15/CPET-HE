...
namespace cpet
{
    bool is_primitive_root(uint64_t root, const PolyModulus& poly_modulus, uint64_t modulus)
    {
        ...
        return pow_mod(root, poly_modulus.degree() >> 1ULL, modulus) == (modulus - 1);
    }
	...
	bool try_primitive_root(const PolyModulus& poly_modulus, uint64_t modulus, uint64_t& destination)
	{
        ...
        do
        {
            ...
            destination = mod((static_cast<uint64_t>(rand()) << 32) | static_cast<uint64_t>(rand()), modulus);      
            destination = pow_mod(destination, quotient_group_size, modulus);
    ...
    bool try_minimal_primitive_root(const PolyModulus& poly_modulus, uint64_t modulus, uint64_t& destination)
    {
        ...
        uint64_t root_sq = mul_mod(root, root, modulus);
        ...
        for (size_t i = 0; i < poly_modulus.degree(); i += 2)
        {
            ...
            candidate_minimal_root = mul_mod(candidate_minimal_root, root_sq, modulus);
        }
    ...
    bool try_inverse_minimal_primitive_root(const PolyModulus& poly_modulus, uint64_t modulus, uint64_t& destination)
    {
        ...
        destination = inverse_mod(root, modulus);
    ...
    void ntt(RnsCycloRing& ring, const std::vector<uint64_t>& omegas)
    {
        ...
        for (uint64_t m = 0; m < ring.basis_size(); m++)
        {
        ...
        auto modulus = ring.basis_begin();
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            for (uint64_t len = 2; len <= poly_modulus.degree(); len <<= 1)
            {
                uint64_t wlen = pow_mod(omegas[m], poly_modulus.degree() / len, *modulus);
                ...
                for (size_t i = 0; i < poly_modulus.degree(); i += len)
                {
                    ...
                    for (size_t j = 0; j < (len >> 1); j++)
                    {
                        ...
                        uint64_t v = mul_mod(ring(m, i + j + (len >> 1)), w, *modulus);

                        ring(m, i + j, add_mod(u, v, *modulus));
                        ring(m, i + j + (len >> 1), sub_mod(u, v, *modulus));
    ...
    void inverse_ntt(RnsCycloRing& ring, const std::vector<uint64_t>& inv_omegas)
    {
        ...
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            uint64_t n_inv = inverse_mod(poly_modulus.degree(), *modulus);

            for (size_t i = 0; i < poly_modulus.degree(); i++)
            {
                ring(m, i, mul_mod(ring(m, i), n_inv, *modulus));
            }
        }
    }
    ...
    void ntt_negacyclic(RnsCycloRing& ring)
    {
        ...
        std::vector<uint64_t> zetas(ring.basis_size());
        const PolyModulus poly_modulus_2n(2 * poly_modulus_n.degree());

        auto modulus = ring.basis_begin();
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            if (!try_minimal_primitive_root(poly_modulus_2n, *modulus, zetas[m]))
            {
        ...
        std::vector<uint64_t> omegas(ring.basis_size());

        modulus = ring.basis_begin();
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            uint64_t zeta_pow = 1;
            ...
            {
                ring(m, i, mul_mod(ring(m, i), zeta_pow, *modulus));
                zeta_pow = mul_mod(zeta_pow, zetas[m], *modulus);
            }

            omegas[m] = pow_mod(zetas[m], 2, *modulus);
        }
    ...
    void inverse_ntt_negacyclic(RnsCycloRing& ring)
    {
        ...
        // Find 2n-th primitive root(ζ).
        std::vector<uint64_t> inv_zetas(ring.basis_size());
        const PolyModulus poly_modulus_2n(2 * poly_modulus_n.degree());

        auto modulus = ring.basis_begin();
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            if (!try_inverse_minimal_primitive_root(poly_modulus_2n, *modulus, inv_zetas[m]))
            {
                throw std::logic_error("Failed to find minimal primitive root for NTT negacyclic.");
            }
        }
        ...
        std::vector<uint64_t> inv_omegas(ring.basis_size());

        modulus = ring.basis_begin();
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            inv_omegas[m] = pow_mod(inv_zetas[m], 2, *modulus);
        }
        ...
        for (uint64_t m = 0; m < ring.basis_size(); ++m, ++modulus)
        {
            ...
            {
                ring(m, j, mul_mod(ring(m, j), inv_zeta_pow, *modulus));
                inv_zeta_pow = mul_mod(inv_zeta_pow, inv_zetas[m], *modulus);
            }
        }
    }
}